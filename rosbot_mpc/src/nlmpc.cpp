...
nlmpc::nlmpc(const ros::NodeHandle &nh, const ros::NodeHandle &private_nh) :
    ...
    dynamic_reconfigure::Server<rosbot_mpc::NonLinearMPCConfig>::CallbackType f_controller;
...
void nlmpc::getTarget(const nav_msgs::Odometry::ConstPtr &waypoint) {
    ...
    state_ = rosbot_fsm_node::RECEIVED;
    if (waypoint->header.frame_id == "emergency") {
        ...
        state_ = rosbot_fsm_node::WAITING;
        ...
        if ((sq_dist >= oscillation_reset_dist_ * oscillation_reset_dist_ ||
            ...
            if (std::pow((targetPose(0) - pos(0)) * (targetPose(0) - pos(0)) +
                ...
                ((nextTarget(2) == -1 &&
                 ...
                 nextTarget(2) == 0)) {
                state_ = rosbot_fsm_node::REACHED;
                reset_dist = 0;
                reset_angle = 0;
                ...
                reset_dist = 0;
                reset_angle = 0;
                state_ = rosbot_fsm_node::OSCILLATING;
            }
        }
    }
}
...
void nlmpc::invalidTimeout() {
    ...
    if (planner_patience_ > 0 && (ros::Time::now() > attempt_end || cnt_failed > uint32_t(max_planning_retries_))) {
        ...
        state_ = rosbot_fsm_node::INVALID;
        if (prev_min_dist == -1) {
...
// get nearest positive/negative yaw to be sure shortest path is chosen
    ...
    if (!receivedTarget) {
        ...
        if (state_ != rosbot_fsm_node::RECEIVED) {
            ...
            if (state_ != rosbot_fsm_node::OPERATING || need_init) {
                if (state_ == rosbot_fsm_node::REACHED) {
        ...
        robState.data = state_;
        pubRobState.publish(robState);
        state_ = rosbot_fsm_node::OPERATING;
    }
...
void nlmpc::ControllerDynConfigCallBack(rosbot_mpc::NonLinearMPCConfig &config, uint32_t level) {
