#include "../include/rosbot_fsm_node.h"
...
void rosbot_fsm_node::mpcStateCallback(const std_msgs::Int8::ConstPtr &msg) {
    emergency = false;
...
/*
void rosbot_fsm_node::rtabmapInfoCallback(const rtabmap_ros::Info::ConstPtr &msg) {
...
void rosbot_fsm_node::general_reset() {
...
rosbot_fsm_node::~rosbot_fsm_node() {}

rosbot_fsm_node::rosbot_fsm_node(ros::NodeHandle nh, ros::NodeHandle private_nh) : nh_(nh),
                                                                                       private_nh_(private_nh),
                                                                                       spinner(4) {

    rosbot_fsm_node::init_vars();

    get_frontier = nh_.serviceClient<aslam_rosbot::get_frontier_list>(get_frontier_topic);
    ...
    if (sim) {
        ...
        exact->registerCallback(boost::bind(&rosbot_fsm_node::odomCallback,this, _1, _2));
    }
    else{
        approx.reset(new sync_approx(MyApproxSyncPolicy (10), odomRobotSub, odomCamSub));
        approx->registerCallback(boost::bind(&rosbot_fsm_node::odomCallback,this, _1, _2));
    }

    // get mpc state real time
    mpcStateClient = nh_.subscribe<std_msgs::Int8>(mpcStateTopic, 1, &rosbot_fsm_node::mpcStateCallback, this);

    //rtabmap_info = nh_.subscribe<rtabmap_ros::Info>("/rtabmap/info", 1, &rosbot_fsm_node::rtabmapInfoCallback, this);
    ...
    get_best_path = nh_.serviceClient<aslam_rosbot::get_best_path>(get_best_path_topic);
    get_best_head = nh_.serviceClient<aslam_rosbot::get_best_head>(get_best_head_topic);
                ...
                if (currentPlan.size() > 1) {
                    if (currentPlan.size() % 2 == 0 && !done_360) {
                        if (!mid_optimizer) {
                            ...
                            aslam_rosbot::get_best_head srv;
...
void rosbot_fsm_node::replan_goal() {
    if (currentPlan.empty()) {
        require_new_goal = true;
        replan = false;
        failed_replans += 1;
...
rosbot_fsm_node::getBestPath(const std::vector<std::pair<std::vector<geometry_msgs::Pose>, double>> &paths, int kind,
                               int fov) {
    aslam_rosbot::get_best_path srv;
    aslam_rosbot::SinglePath sp;
...
std::vector<geometry_msgs::Pose> rosbot_fsm_node::getFrontierPoints() {
    aslam_rosbot::get_frontier_list srv;
...
std::vector<geometry_msgs::PoseStamped> rosbot_fsm_node::revisit_goal() {
...
std::vector<geometry_msgs::Pose>
rosbot_fsm_node::refinePlan(std::vector<geometry_msgs::PoseStamped> plan, double &tot_dist, double thr) {
...
rosbot_fsm_node::getPlan(const geometry_msgs::Pose end, const geometry_msgs::PosePtr start) {
...
void rosbot_fsm_node::init_vars() {
    tf_ok = false;
    require_new_goal = true;
    emergency = false;
...
void rosbot_fsm_node::transform_pose(geometry_msgs::Pose &pose) {
...
void
rosbot_fsm_node::transform_odom(const nav_msgs::Odometry::ConstPtr &odomRobot,
...
void rosbot_fsm_node::odomCallback(const nav_msgs::Odometry::ConstPtr &odomRobot,
                                     const nav_msgs::Odometry::ConstPtr &odomCam) {
...
int main(int argc, char **argv) {
    ros::init(argc, argv, "rosbot_fsm");
    ros::NodeHandle nh;
    ros::NodeHandle private_nh("~");

    rosbot_fsm_node rosbotFsmNode(nh, private_nh);
    return 0;
}
