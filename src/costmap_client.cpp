...
/*
Please refer to http://docs.ros.org/en/jade/api/explore_lite/html/costmap__client_8cpp_source.html for the orgional implemation
*/
...
namespace frontier_exploration {
    ...
    {
        std::array<uint8_t, 256> cost_translation_table = {};

        for (size_t i = 0; i < cost_translation_table.size(); i++)
        {
            cost_translation_table[i] = static_cast<uint8_t>(1 + (251 * (i - 1)) / 97); // ???
        }
        ...
        cost_translation_table[static_cast<uint8_t>(-1)] = 255; // UNKNOWN
        ...
        COST_TRANSLATION_TABLE(initTranslationTable())
    {
        ...
        pnh.param("robot_base_frame", _robotBaseFrame, std::string("base_link")); //setting the node parameter
        // transform tolerance is used for all tf transforms here
        pnh.param("transform_tolerance", _tfTolerance, 0.3);  // string, value, default value
        ...
        std::string tfError;
        while (ros::ok() && !_tf->waitForTransform(_globalFrame, _robotBaseFrame, ros::Time(), ros::Duration(0.1), ros::Duration(0.01), &tfError))
        {
            ros::spinOnce();
    ...
    geometry_msgs::Pose Costmap2DClient::getRobotPose() const
    {
        ...
        {
            ROS_ERROR_THROTTLE(1.0, "Connectivity Error looking up robot pose: %s\n", ex.what()); // throtelled ourput will print messages every "time specified in seconds"
            return {};
