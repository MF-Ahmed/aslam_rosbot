...
from maslam_rosbot.msg import Point2D, ChooseGoalResult, ChooseGoalFeedback, ChooseGoalAction
...
class ChoosePointsServer:
    ...
    def select_points(self, matrix_, num_rows):
        ...
        p.x = matrix[max_reward_idx, 1]
        p.y = matrix[max_reward_idx, 2]
        ...
        for c in range(len(chosen_coords)):
            for g in range(len(matrix)):
                ...
                if matrix[g,0] != -np.inf:
                    # Compute the distance from the goals already chosen and all the possible other goals
                    # Normalise the distance with respect to the max reward and the max distance
                    d = np.sqrt(pow(chosen_coords[c].x-matrix[g,1],2)+pow(chosen_coords[c].y-matrix[g,2],2))
                    if d != 0.0:
                        # Update the reward with a -k/d coefficient normalized with respect to the max distance
                        matrix[g,0] -= (K/d)
                    else:
                        matrix[g,0] = -np.inf

        # Print the new matrix values
        # Convert the matrix to a string representation
        new_matrix_str = "\n".join(["\t".join([str(element) for element in row]) for row in matrix])
        rospy.loginfo("New Reward matrix:\n%s", new_matrix_str)
