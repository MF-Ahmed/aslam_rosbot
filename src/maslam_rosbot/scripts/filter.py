...
from maslam_rosbot.msg import PointArray

from visualization_msgs.msg import Marker
from nav_msgs.msg import OccupancyGrid
from geometry_msgs.msg import Point, PointStamped
from dynamic_reconfigure.server import Server
from maslam_rosbot.cfg import informationGainConfig
...
def node():
    ...
    if (robot_frame == 'robot_0/base_link'):
        points_clust = createMarker(namespace, frame=map_data_.header.frame_id, ns="filtered_frontiers", colors=[239.0, 41.0, 41.0],scale=0.2)
    elif (robot_frame == 'robot_1/base_link'):
        points_clust = createMarker(namespace, frame=map_data_.header.frame_id, ns="filtered_frontiers", colors=[114.0, 159.0, 207.0],scale=0.2) 
    elif (robot_frame == 'robot_2/base_link'):
        points_clust = createMarker(namespace, frame=map_data_.header.frame_id, ns="filtered_frontiers", colors=[92.0, 53.0, 102.0],scale=0.2)       


    #points_clust = createMarker(namespace, frame=map_data_.header.frame_id, ns="filtered_frontiers", colors=[200.0, 200.0, 200.0],scale=0.2)
    #points_clust = createMarker(namespace, frame=map_data_.header.frame_id, ns="filtered_frontiers", colors=[50.0, 50.0, 50.0],scale=0.2) 





    #points_clust = createMarker(namespace, frame=map_data_.header.frame_id, ns="filtered_frontiers", colors=[0.0, 255, 0.0])
