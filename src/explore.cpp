#include "aslam_turtlebot/explore.h"
...
namespace frontier_exploration {
    ...
    void Explore::visualizeFrontiers(const std::vector<Frontier>& frontiers)
    {
        ...
        for (auto& frontier : frontiers)
        {
            ...
            //ROS_INFO("Frontier centroid is  = [%.2f,%.2f]",frontier.centroid.x,frontier.centroid.y);
    ...
    void Explore::makePlan()
    {
        ...
        // here frontier is an iterator 
        auto frontier = std::find_if_not(frontiers.begin(), frontiers.end(), [this](const Frontier& f) { return goalOnBlacklist(f.centroid); });
        ...
        ROS_INFO("chosen Frontier is  = [%.2f,%.2f]",frontier->centroid.x,frontier->centroid.y);
