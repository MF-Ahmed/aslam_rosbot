...
def frontiersCallBack(data):
    ...
    for point in data.points:
        frontiers_.append(np.array([round(point.x, 2), round(point.y, 2)]))
...
def node():
    ...
    server_status = [False] * num_robots
    ...
    while not rospy.is_shutdown():
        ...
        # Print log
        rospy.loginfo(f'{robot_name} is waiting for server to allow it to go')
        # Check the server allows the robot to use it
        while not server_status[int(robot_name.split("_")[1])]:
            continue

        rospy.loginfo(f'{robot_name} received server approvation. Now going')
        ...
        if m < 1 or centroids == 0:
            ...
            graph_start_publisher.publish(graph_started)
        else:  # Otherwise
            rospy.loginfo(
        ...
        # The previous goal has been successfully reached
            ...
            if goal_coordinates != []:
                rospy.loginfo(namespace + ": COORDINATES RECEIVED \n" + str(goal_coordinates))
