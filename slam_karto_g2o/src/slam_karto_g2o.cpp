        ...
        marker_count_(0)
{
  ...
  if(private_nh_.getParam("graph_dir", graph_dir_))
  { // graph_dir_ = "/home/julio/source/active_slam_project/graph.g2o";
    solver_->setGraphDir(graph_dir_);
  }
...
void SlamKarto::publishTransform()
{
  ...
  ros::Time tf_expiration = ros::Time::now() + ros::Duration(0.10);
...
void SlamKarto::publishGraphVisualization()
{
  ...
  // std::vector<Eigen::VectorXd > fim;
  // solver_->getGraphWithFIM(graph_nodes, graph_edges, fim);
  //
  // for(unsigned i = 0; i< fim.size(); ++i) {
  //   std::cout << "fim[" << i << "]: " << fim[i] << std::endl;
  // }
